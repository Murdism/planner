...
def add_coordinates(Gnode1: Gnode, Gnode2: Gnode):
    new_Gnode = Gnode()
    new_Gnode.x = Gnode1.x + Gnode2.x
    new_Gnode.y = Gnode1.y + Gnode2.y
    new_Gnode.cost = Gnode1.cost + Gnode2.cost
    return new_Gnode
...
class DStarLite:
    ...
    def __init__(self, ox: list, oy: list):
        ...
        self.obstacles = [Gnode(x - self.x_min_world, y - self.y_min_world)
                          for x, y in zip(ox, oy)]
        self.obstacles_xy = {(obstacle.x, obstacle.y) for obstacle in self.obstacles}
        self.start = Gnode(0, 0)
        self.goal = Gnode(0, 0)
    ...
    def is_obstacle(self, Gnode: Gnode):
        is_in_obstacles = (Gnode.x, Gnode.y) in self.obstacles_xy
        is_in_detected_obstacles = (Gnode.x, Gnode.y) in self.detected_obstacles_xy
        return is_in_obstacles or is_in_detected_obstacles

    def c(self, Gnode1: Gnode, Gnode2: Gnode):
        if self.is_obstacle(Gnode2):
            # Attempting to move from or to an obstacle
            return math.inf
        new_Gnode = Gnode(Gnode1.x-Gnode2.x, Gnode1.y-Gnode2.y)
        detected_motion = list(filter(lambda motion:
                                      compare_coordinates(motion, new_Gnode),
                                      self.motions))
    ...
    def update_vertex(self, u: Gnode):
        if not compare_coordinates(u, self.goal):
            self.rhs[u.x][u.y] = min([self.c(u, sprime) +
                                      self.g[sprime.x][sprime.y]
                                      for sprime in self.succ(u)])
        if any([compare_coordinates(u, Gnode) for Gnode, key in self.U]):
            self.U = [(Gnode, key) for Gnode, key in self.U
                      if not compare_coordinates(Gnode, u)]
    ...
    def detect_changes(self):
        ...
        if random.random() > 1 - p_create_random_obstacle:
            ...
            changed_vertices.append(Gnode(x, y))
    ...
    def compute_current_path(self):
        path = list()
        current_point = Gnode(self.start.x, self.start.y)
    ...
    def compare_paths(self, path1: list, path2: list):
        if len(path1) != len(path2):
            return False
        for Gnode1, Gnode2 in zip(path1, path2):
            if not compare_coordinates(Gnode1, Gnode2):
                return False
        return True

    def display_path(self, path: list, colour: str, alpha: float = 1.0):
        px = [(Gnode.x + self.x_min_world) for Gnode in path]
        py = [(Gnode.y + self.y_min_world) for Gnode in path]
        drawing = plt.plot(px, py, colour, alpha=alpha)
        plt.pause(pause_time)
        return drawing

    def main(self, start: Gnode, goal: Gnode,
             spoofed_ox: list, spoofed_oy: list):
        self.spoofed_obstacles = [[Gnode(x - self.x_min_world,
                                        y - self.y_min_world)
...
def main():
    ...
    dstarlite.main(Gnode(x=sx, y=sy), Gnode(x=gx, y=gy),
