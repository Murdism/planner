...
def add_coordinates(node1: GridNode, node2: GridNode):
    new_node = GridNode()
...
def compare_coordinates(node1: GridNode, node2: GridNode):
...
class DStarLite:
    ...
    def __init__(self, ox: list, oy: list):
        # Ensure that within the algorithm implementation all GridNode coordinates
        ...
        self.obstacles = [GridNode(x - self.x_min_world, y - self.y_min_world)
    ...
    def is_obstacle(self, GridNode: GridNode):
        is_in_obstacles = (GridNode.x, GridNode.y) in self.obstacles_xy
        is_in_detected_obstacles = (GridNode.x, GridNode.y) in self.detected_obstacles_xy
        return is_in_obstacles or is_in_detected_obstacles

    def c(self, node1: GridNode, node2: GridNode):
        if self.is_obstacle(node2):
            # Attempting to move from or to an obstacle
            return math.inf
        new_node = GridNode(node1.x-node2.x, node1.y-node2.y)
    ...
    def compute_current_path(self):
        path = list()
        current_point = GridNode(self.start.x, self.start.y)
    ...
    def main(self, start: GridNode, goal: GridNode,
             spoofed_ox: list, spoofed_oy: list):
        self.spoofed_obstacles = [[GridNode(x - self.x_min_world,
                                        y - self.y_min_world)
...
def main():
    ...
    dstarlite.main(GridNode(x=sx, y=sy), GridNode(x=gx, y=gy),
