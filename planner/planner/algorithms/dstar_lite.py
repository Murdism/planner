...
class DStarLite:
    ...
    def update_vertex(self, node: GridNode):
        """Recompute rhs[node] and ensure PQ membership matches g!=rhs (like ref impl)."""
        if node != self.goal:
            # compute rhs = min_{succ} (cost(node, succ) + g(succ))
            best = math.inf
            for succ in self.get_successors(node):
                c = self.cost(node, succ)
                if c < math.inf:
                    val = c + self.get_g(succ)
                    if val < best:
                        best = val
            self.set_rhs(node, best)

        # remove stale entry from PQ (safe even if not present)
    ...
    def compute_shortest_path(self):
        ...
        while (not self.U.empty()):
            ...
            if (not self.compare_keys(top, start_key)) and (self.get_rhs(self.start) == self.get_g(self.start)):
                break

            iterations += 1
            try:
                u, k_old = self.U.pop_task()
            except KeyError:
                break
            ...
            if self.compare_keys(k_old, k_new):
                ...
                self.U.add_or_update_task(u, k_new)
                ...
                # g <= rhs -> set to inf and update predecessors + node
                self.set_g(u, math.inf)
                for pred in self.get_predecessors(u):
                    self.update_vertex(pred)
                self.update_vertex(u)
        if iterations > 0:
            print(f"compute_shortest_path completed in {iterations} iterations; U size: {self.U.size()}")
