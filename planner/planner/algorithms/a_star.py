...
from utils.dynamic import calc_distance_heuristic
...
def sec_main():
    print("Start Hybrid A* planning (dynamic map test)")
    plt.ion()
    ...
    ox, oy = [p[0] for p in keep], [p[1] for p in keep]
    plan_and_draw(start, goal, XY_GRID_RESOLUTION,ox, oy, title_note="(- removed inner wall @ x=20)")
...
def plan_and_draw(start, goal, XY_GRID_RESOLUTION, ox, oy, title_note=""):
    """
    Run hybrid A* planning and draw the path and obstacles.
    Can be called multiple times with updated obstacles.
    """
    if show_animation:
        plt.cla()
        plt.plot(ox, oy, ".k")
